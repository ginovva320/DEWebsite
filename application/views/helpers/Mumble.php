<?php
class Zend_View_Helper_Mumble {
	function mumble() {
		$dataUrl =  'http://api.commandchannel.com/cvp.json?email=matthew.schlitt@gmail.com&apiKey=bf825109-6813-4410-813a-5aec4317210c';
		return $this->render( $dataUrl, 'json' );
	}
	...
	*/
	protected static function renderUser($user) {
		...
		if ($user["mute"])
			...
			*/
		if ($user["selfMute"])
			$output .= "<span class='mumbleChannelViewer-selfMuted'>{$user["name"]}</span></li>";
		else
